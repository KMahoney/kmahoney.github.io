...
/***/ function(module, exports, __webpack_require__) {
	...
	var blue_square = context.createTexture(200, 200);
	...
	var happy = context.createTexture(100, 100);
	...
	function buildScene(context) {
	    ...
	    var interp = transformers_1.interpolators.inverseCubic;
	    // A blue square roams the perimeter
	    var sprites = [
	        new funscene_1.Sprite(blue_square, 
	        // move around the edge and repeat
	        transformers_1.repeat([
	            transformers_1.move(interp, 1000, 0, 0, 800, 0),
	            transformers_1.move(interp, 1000, 800, 0, 800, 800),
	            transformers_1.move(interp, 1000, 800, 800, 0, 800),
	            transformers_1.move(interp, 1000, 0, 800, 0, 0),
	        ])),
	    ];
	    // A grid of happy faces!
	    for (var i = 0; i < 100; i++) {
	        sprites.push(new funscene_1.Sprite(happy, transformers_1.combine([
	            // order them in to a grid
	            transformers_1.translate(100 * Math.floor(i / 10), Math.floor(i % 10) * 100),
	            transformers_1.sequence([
	                // they take turns to...
	                transformers_1.wait(i * 1000),
	                transformers_1.combine([
	                    // gradually turn red
	                    transformers_1.blend(interp, 6000, [1, 1, 1, 1], [1, 0, 0, 1]),
	                    // rotate around their middle
	                    transformers_1.translate(50, 50),
	                    transformers_1.repeat([
	                        transformers_1.rotate(interp, 1000, 0, 1)
	                    ]),
	                    transformers_1.translate(-50, -50),
	                ])
	            ])
	    ...
	    sprites.push(new funscene_1.Sprite(blue_square, transformers_1.combine([
	        // Put it in the middle
	        transformers_1.translate(1000 / 2 - 100, 1000 / 2 - 100),
	        // fade in and out
	        transformers_1.repeat([
	            transformers_1.fade(interp, 1000, 0, 1),
	            transformers_1.fade(interp, 1000, 1, 0),
	        ])
	    ])));
	    // The camera
	    var world = transformers_1.combine([
	        // focus on the middle
	        transformers_1.translate(context.width / 2 - 500, context.height / 2 - 500),
	        transformers_1.repeat([
	            // wait 6 seconds
	            transformers_1.wait(6000),
	            // spin around the centre!
	            transformers_1.combine([
	                transformers_1.translate(500, 500),
	                transformers_1.rotate(interp, 3000, 0, 1),
	                transformers_1.translate(-500, -500),
	            ])
	        ])
	    ]);
	    return new funscene_1.Scene(world, sprites);
	}
	// Run the scene
