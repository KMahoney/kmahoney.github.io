...
The application itself is nothing revolutionary: a keyboard-driven 'todo' app with a minimal UI and the ability to synchronise with other services. I got fed up with notifications from multiple applications that required my attention, and I wanted them all in one place.
...
<div class="overlay"></div>
{% highlight tsx %}
// Definte the application's state as a global variable
...
export function mutate(f: (state: State) => State) {
  ...
  state = f(state);
  for (const subsciber of subscribers) {
    subsciber();
  }
...
The entire system can be built locally and then copied across to the remote machine.
